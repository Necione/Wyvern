...
commandManager.handleCommandOnCooldown((i, command, timeLeft) => {
  ...
  if (command.name === "heavy") {
    embed.setDescription(`\`${WARNING}\` Your heavy attack is on cooldown! Please wait **${seconds}s**`)
  } else {
    embed.setDescription(`This command is on cooldown. Please wait for **${hours}h ${minutes}m ${seconds}s**`)
  }
...
client.on("ready", async () => { 
  ...
  if (!guild)
    throw new CommandError(`Cannot find guild "${guildId}"`);
